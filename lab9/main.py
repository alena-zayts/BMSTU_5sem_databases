...
def get_dep1_workers(cur):
    ...
    cur.execute("select * from workers where department_id = 1")
    res = cur.fetchall()
...
def task_02(cur, id):
    ...
    cur.execute(f"select * from workers where department_id = {id}")
...
def dont_do(cur):
    ...
    t1 = time()
    cur.execute("select * from workers where worker_id = 1")
    t2 = time()
...
def del_worker(cur, con):
    ...
    t1 = time()
    cur.execute(f"delete from workers where worker_id = {wid};")
    t2 = time()
...
def ins_worker(cur, con):
    ...
    t1 = time()
    cur.execute(f"insert into workers (department_id, first_name, second_name, experience) "
                f"values (1, 'AAA', 'BBB', 2);")
    t2 = time()

    cur.execute(f"select * from workers where worker_id = {wid}")
    result = cur.fetchall()
...
def upd_worker(cur, con):
    ...
    t1 = time()
    cur.execute(f"UPDATE workers SET department_id = 1 WHERE worker_id = {wid}")
    t2 = time()
...
if __name__ == '__main__':
    ...
    print("1. Работники 1 отдела (задание 2)\n"
